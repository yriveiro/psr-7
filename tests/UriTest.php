...
class UriTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $uri = new Uri('https://user:pass@local.example.com:3001/foo?bar=baz#quz');
        $new = $uri->withUserInfo('jdoe');
        $this->assertNotSame($uri, $new);
        $this->assertEquals('jdoe', $new->getUserInfo());
        $this->assertEquals('https://jdoe@local.example.com:3001/foo?bar=baz#quz', (string) $new);
    }

    public function testWithUserInfoReturnsNewInstanceWithProvidedUserAndPassword()
    {
        $uri = new Uri('https://user:pass@local.example.com:3001/foo?bar=baz#quz');
        $new = $uri->withUserInfo('jdoe', 'no-name');
        $this->assertNotSame($uri, $new);
        $this->assertEquals('jdoe:no-name', $new->getUserInfo());
        $this->assertEquals('https://jdoe:no-name@local.example.com:3001/foo?bar=baz#quz', (string) $new);
    }
